import { createFileRoute } from "@tanstack/react-router";
import { useMutation } from "@tanstack/react-query";
import { useState } from "react";
import { toast } from "sonner";
import { api, type ReviewResult } from "@/lib/lifeos-api";
import { AppShell } from "@/components/lifeos/app-shell";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";

export const Route = createFileRoute("/review")({
  component: ReviewPage,
});

function ReviewPage() {
  const [week, setWeek] = useState<string>("");
  const [result, setResult] = useState<ReviewResult | null>(null);

  const preview = useMutation({
    mutationFn: () => api.previewReview(week || undefined),
    onSuccess: (r) => { setResult(r); toast.success("Preview ready"); },
    onError: (e: Error) => toast.error(e.message),
  });
  const write = useMutation({
    mutationFn: () => api.writeReview(week || undefined),
    onSuccess: (r) => { setResult(r); toast.success(`Written to ${r.path}`); },
    onError: (e: Error) => toast.error(e.message),
  });

  return (
    <AppShell>
      <h1 className="text-2xl font-semibold mb-6">Weekly review</h1>
      <div className="flex items-end gap-3 mb-4">
        <div>
          <Label htmlFor="w">Week start (Monday)</Label>
          <Input id="w" type="date" value={week} onChange={(e) => setWeek(e.target.value)} />
        </div>
        <Button onClick={() => preview.mutate()} disabled={preview.isPending} variant="secondary">
          {preview.isPending ? "Loading…" : "Preview review"}
        </Button>
        <Button onClick={() => write.mutate()} disabled={write.isPending}>
          {write.isPending ? "Writing…" : "Write to vault"}
        </Button>
      </div>
      {result && (
        <div className="space-y-3">
          <Alert>
            <AlertTitle>{result.written ? "Written" : "Preview"}</AlertTitle>
            <AlertDescription>
              {result.path} · week of {result.week_start}
            </AlertDescription>
          </Alert>
          <pre className="border rounded-md p-4 text-xs whitespace-pre-wrap bg-muted/30 overflow-auto max-h-[70vh]">
            {result.content}
          </pre>
        </div>
      )}
    </AppShell>
  );
}
